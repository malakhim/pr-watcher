...
# Add bundled libs to Python path (Platypus bundles this script under Resources)
...
class PRWatcher(rumps.App):
    def __init__(self):
        ...
        # queue for passing results from background thread to main thread
        ...
        # poll timer: fires refresh every POLL_INTERVAL seconds
        self.timer = rumps.Timer(self.refresh_prs, POLL_INTERVAL)
        self.timer.start()

        # apply timer: checks for results from the background thread at 0.5s intervals
        self._apply_timer = rumps.Timer(self._apply_pending, 0.5)
        self._apply_timer.start()

        if not self.repo:
            # defer so the run loop can start first
    ...
    def _ensure_status_icons(self):
        """Generate simple colored PNG icons for each build state if they don't exist."""
        import struct, zlib
        ...
        icons = {
            "icon_green.png":  (52, 199, 89),   # macOS system green
            "icon_yellow.png": (255, 204, 0),   # macOS system yellow
            "icon_red.png":    (255, 59, 48),   # macOS system red
        }
    ...
    def detect_gh_user(self):
        """Return authenticated gh username or None."""
    ...
    def call_github(self, endpoint):
        """Call gh api for repos/<repo>/<endpoint> and return parsed JSON or None."""
        if not GH or not os.path.exists(GH):
    ...
    def refresh_prs(self, _):
        """Called by the poll timer on the main thread — just spawns the background worker."""
        threading.Thread(target=self._do_refresh, daemon=True).start()

    def _do_refresh(self):
        """Runs on a background thread. Puts results into the queue for the main thread to apply."""
        try:
    ...
    def _apply_pending(self, _):
        """Called by the apply timer on the main thread — drains the result queue and updates the UI."""
        try:
    ...
    def update_menu(self, prs):
        ...
        # add manual refresh and settings
        self.menu.add(rumps.MenuItem("Refresh now", callback=lambda _: self.refresh_prs(None)))
        self.menu.add(rumps.MenuItem(f"Set Repo… ({self.repo or 'not set'})", callback=self.set_repo))
        self.menu.add(None)  # separator
        self.menu.add(rumps.MenuItem("Quit", callback=rumps.quit_application))

        # update menu bar icon based on aggregate check status
        if prs:
    ...
    def fetch_check_status(self, sha):
        # Prefer check-runs endpoint for richer info, fallback to commit status
        if not sha:
            return "pending", "🟡"
        # check-runs
        runs = self.call_github(f"commits/{sha}/check-runs")
        if runs and isinstance(runs, dict):
            # API returns {"total_count": N, "check_runs": [...]}
            check_runs = runs.get("check_runs", [])
            if check_runs:
                # If any run failed -> failure; if all completed and conclusions are success -> success
                ...
                if len(conclusions) == len(check_runs) and all(c in ("success", "skipped", "neutral") for c in conclusions):
                    return "success", "🟢"
                # some runs still in progress or mixed
                return "pending", "🟡"
    ...
    def fetch_review_status(self, pr_number):
        ...
        if not reviews:
            return "none", "👀"
        # Track the latest actionable review per reviewer (ignore COMMENTED)
        latest_by_user = {}
    ...
    def is_assigned_or_requested(self, pr):
        """Return True if the authenticated user is the author, an assignee, or a requested reviewer."""
        if not self.gh_user:
            return False
        # assignees
        ...
        if any(a.get("login") == self.gh_user for a in assignees):
            return True
        # requested reviewers (pulls API returns 'requested_reviewers')
        requested = pr.get("requested_reviewers", []) or []
        if any(r.get("login") == self.gh_user for r in requested):
            return True
        # also consider if you're the author (optional)
    ...
    def fetch_my_prs(self):
        ...
        for pr in raw:
            ...
            # PR head SHA is required for commit status / check-runs
        ...
        # sort by number or updated_at if you prefer
